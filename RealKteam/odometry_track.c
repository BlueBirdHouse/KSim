...
int odometry_track_start_pos(struct sOdometryTrack * ot, unsigned long pos_left, unsigned long pos_right) {
	...
	ot->configuration.wheel_conversion_left = 2 * PI * 0.0205 / 4198.4;
	ot->configuration.wheel_conversion_right = 2 * PI * 0.0205 / 4198.4;
...
void odometry_track_step_pos(struct sOdometryTrack * ot, unsigned long pos_left, unsigned long pos_right) {
	...
	//printf("X=%f",ot->result.x);
	//printf("Y=%f",ot->result.y);
	//printf("theta=%f",ot->result.theta);

	//printf("posleft=%lu",pos_left);
	//printf("posright=%lu",pos_right);
}
