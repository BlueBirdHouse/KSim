function msfun_khepera3_WithPosition(block)
...
%         robot = customData('robotHandle');
        ...
        %由于使用了部分接近传感器模拟超声波传感器，所以这里的输出端口不够了
        %block.OutputPort(1).Data = robot.ir_array.get_range();
        Temp = robot.ir_array.get_range();
        block.OutputPort(1).Data = Temp(1,1:9);
        
        %不用老是动态显示机器人的编码器信息
        robot.encoders.ticks;
        block.OutputPort(2).Data = [robot.encoders.ticks];
        block.OutputPort(3).Data = [robot.robotKhepera3_pose.x ; robot.robotKhepera3_pose.y ; robot.robotKhepera3_pose.theta];
        
        %测试端口输出代码
        block.OutputPort(4).Data = [robot.Filter.Pose];
        %Temp = robot.dynamics.Dynamics;
        %Temp = 0;
    end %end Outputs
...
%         robot = customData('robotHandle');
        ...
        %下面是我的参数
        robot.Filter.User_ShowObject = block.DialogPrm(4).Data;
